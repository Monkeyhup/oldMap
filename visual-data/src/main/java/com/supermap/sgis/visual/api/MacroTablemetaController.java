...
//        }
    ...
    /**
     * 通过parid查询子内容
     * songxiaomin 2017/3/7
     */
    @RequestMapping(value="/Types/{id}/findByParid",method = RequestMethod.GET)
    @ResponseBody
    public List<TableMetadata> findByParid(@PathVariable("id") int parid){
       // List<TableMetadata> list=macroTablemetaService.findByParid(parid);
        //TableMetadata[]tt= list.toArray(new TableMetadata[list.size()]);
        return macroTablemetaService.findByParid(parid);
