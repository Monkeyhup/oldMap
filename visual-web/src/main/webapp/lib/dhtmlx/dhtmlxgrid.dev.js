...
function dhtmlXGridObject(a) {
    ...
    this.init = function (e) {
        ...
        if (this._hstyles) {
            ...
            b._childIndexes = [];
            for (var d = 0, f = 0; f < this.hdrLabels.length; f++) b._childIndexes[f] = f - d, this.hdrLabels[f] == this.splitSign && f != 0 ? (_isKHTML && b.insertCell(f - d), b.cells[f - d - 1].colSpan = (b.cells[f - d - 1].colSpan || 1) + 1, b.childNodes[f - d - 1]._cellIndex++, d++, b._childIndexes[f] = f - d) : (b.insertCell(f - d), b.childNodes[f - d]._cellIndex = f, b.childNodes[f - d]._cellIndexS = f, this.setColumnLabel(f, this.hdrLabels[f]));
    ...
    this.setColumnSizes = function (e) {
        for (var a = 0, b = [], c = 0, d = 0; d < this._cCount; d++) if (this.initCellWidth[d] == "*" && !this._hrrar[d]) this._awdth = !1, b.push(d);
        else {
    ...
    this.enableMultiline = function (e) {
        this.multiLine = convertStringToBoolean(e)
    };
    ...
    this.changeCursorState = function (e) {
        ...
        if (a) {
            ...
            var b = (e.layerX || 0) + (!_isIE && e.target.tagName == "DIV" ? a.offsetLeft : 0);
    ...
    this.setSortImgState = function (a, b, c, f) {
        ...
        if (convertStringToBoolean(a)) {
            ...
            if (d) {
                for (var h = 0; h < d.childNodes.length; h++) if (d.childNodes[h]._cellIndexS == b) return this.r_fldSorted = d.childNodes[h], this.setSortImgPos();
    ...
    this.doClick = function (a, b, d, f) {
        ...
        if (a.tagName == "TD") {
            ...
            if (d == 1) {
                ...
                for (var p = 0; p < this.rowsCol.length; p++) if (p >= n && p <= o) if (this.rowsCol[p] && !this.rowsCol[p]._sRow) this.rowsCol[p].className.indexOf("rowselected") == -1 && this.callEvent("onBeforeSelect", [this.rowsCol[p].idd, g]) && (this.rowsCol[p].className += " rowselected", this.selectedRows[this.selectedRows.length] = this.rowsCol[p]);
    ...
    this.moveToVisible = function (a, b) {
        ...
        try {
            ...
            else d < this.objBox.scrollTop && (k = d - 5);
    ...
    this.copyRowContent = function (a, b) {
        ...
        else for (var d = 0; d < c.cells.length; d++) this.cells(b, d).setValue(this.cells(a, d).getValue());
...
dhtmlXGridObject.prototype = {
        ...
        if (d) {
            ...
            if (b && d.style.display != "none") {
                ...
                if (e != -1) {
                    ...
                    for (var j = 0; j < d.childNodes.length; j++) d.childNodes[j].className = d.childNodes[j].className.replace(/cellselected/g, "");
    ...
    _parseHead: function (a) {
        ...
        if (b.length) {
            var c = this.xmlLoader.doXPath("./column", b[0]),
                ...
                e = "setInitWidths",
                j = !1;
            if (d[0]) for (var i = 0; i < d[0].childNodes.length; i++) switch (d[0].childNodes[i].tagName) {
                case "colwidth":
        ...
        if (r.length > 0) {
            ...
            for (k = 0; k < r.length; k++) {
                ...
                this.UserData.gridglobaluserdata.put(r[k].getAttribute("name"), q)
            }
        }
    },
    ...
    attachHeader: function (a, b, c) {
        ...
        c = c || "_aHead";
        if (this.hdr.rows.length) if (a) this._createHRow([a, b], this[c == "_aHead" ? "hdr" : "ftr"]);
        else {
    ...
    _createHRow: function (a, b) {
        ...
        for (i = 0; i < a[0].length; i++) if (a[0][i] == "#cspan") {
            ...
            for (var h = b.rows.length - 2, l = !1, g = null; !l;) {
                for (var g = b.rows[h], k = 0; k < g.cells.length; k++) if (g.cells[k]._cellIndex == i) {
                    l = k + 1;
                    break
                }
                h--
            ...
            else if (m.innerHTML = this.forceDivInHeader ? "<div class='hdrcell'>" + (a[0][i] || "&nbsp;") + "</div>" : a[0][i] || "&nbsp;", (a[0][i] || "").indexOf("#") != -1) if (d = a[0][i].match(/(^|{)#([^}]+)(}|$)/)) {
    ...
    _process_xml_row: function (a, b) {
        ...
        if (this._ud_enabled) for (var e = this.xmlLoader.doXPath("./userdata", b), j = e.length - 1; j >= 0; j--) {
    ...
    _process_xml: function (a) {
        ...
        if (c.tagName == this.xml.top) {
            ...
            for (var g = 0; g < e.length; g++) if (!this.rowsBuffer[g + j]) {
    ...
    _sortCore: function (a, b, c, d, e) {
        ...
        if (b == "str") e[j](function (a, b) {
            return c == "asc" ? d[a.idd] > d[b.idd] ? 1 : d[a.idd] < d[b.idd] ? -1 : 0 : d[a.idd] < d[b.idd] ? 1 : d[a.idd] > d[b.idd] ? -1 : 0
        });
    ...
    _reset_view: function (a) {
        if (this.obj.rows[0]) {
            ...
            else if (_isIE) for (d = b.childNodes.length - 1; d >= 0; d--) b.childNodes[d].removeNode(!0);
            else b.innerHTML = "";
            b.appendChild(c);
            this.rowsCol = dhtmlxArray();
            this._sst && this.enableStableSorting(!0);
...
(function () {
    ...
    function e(a, b) {
        ...
        for (var d = 1, e = [], k = 0; k < b.length; k++) if (typeof b[k] == "object" && b[k].length) for (var j = 0; j < b[k].length; j++) c(k, j, b[k][j]);
        else c(k, 0, b[k]);
        for (k = 0; k < e.length; k++) for (j = 0; j < e[0].length; j++) {
            var n = e[k][j];
            e[k][j] = (n || "").toString() || "&nbsp;";
            if (n && n.colspan) for (var o = 1; o < n.colspan; o++) c(j + o, k, "#cspan");
            if (n && n.rowspan) for (o = 1; o < n.rowspan; o++) c(j, k + o, "#rspan")
        }
        this.setHeader(e[0]);
